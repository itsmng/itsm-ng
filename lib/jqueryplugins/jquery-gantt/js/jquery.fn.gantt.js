  ...
  $.fn.gantt = function (options) {
    ...
    var core = {
      ...
      leftPanel: function (element) {
        ...
        $.each(element.data, function (i, entry) {
          if (i >= element.pageNum * settings.itemsPerPage && i < element.pageNum * settings.itemsPerPage + settings.itemsPerPage) {
            ...
            entries.push('<div class="row name row' + i + (entry.desc ? "" : " fn-wide " + dataId) + '" id="rowheader' + i + '" data-offset="' + (i % settings.itemsPerPage) * tools.getCellSize() + '">' + '<span class="fn-label' + (entry.cssClass ? " " + entry.cssClass : "") + '">' + (entry.name || "") + "</span>" + "</div>");

            if (entry.desc) {
              entries.push('<div class="row desc row' + i + ' " id="RowdId_' + i + dataId + '">' + '<span class="fn-label' + (entry.cssClass ? " " + entry.cssClass : "") + '">' + entry.desc + "</span>" + "</div>");
            }
          }
        });
        ...
        switch (settings.scale) {
          ...
          case "hours":
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              if (rfy !== year) {
                yearArr.push('<div class="row header year" style="width: ' + tools.getCellSize() * scaleUnitsThisYear + 'px;"><div class="fn-label">' + year + "</div></div>");
              ...
              if (rm !== month) {
                monthArr.push('<div class="row header month" style="width: ' + tools.getCellSize() * scaleUnitsThisMonth + 'px"><div class="fn-label">' + settings.months[month] + "</div></div>");
          ...
          case "weeks":
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              if (week > (week = rday.getWeekOfYear())) {
                ...
                diff /= 7;
                yearArr.push('<div class="row header year" style="width: ' + tools.getCellSize() * (scaleUnitsThisYear - diff) + 'px;"><div class="fn-label">' + year + "</div></div>");
                year++;
              ...
              if (rday.getMonth() !== month) {
                ...
                //diff -= (diff <= 6) ? 0 : 7;
                diff /= 7;
                monthArr.push('<div class="row header month" style="width:' + tools.getCellSize() * (scaleUnitsThisMonth - diff) + 'px;"><div class="fn-label">' + settings.months[month] + "</div></div>");
                month = rday.getMonth();
      ...
      navigation: function (element) {
        ...
        if (settings.navigate === "scroll") {
            ...
            $('<div class="nav-slider" />')
              .append(
                $('<div class="nav-slider-left" />')
                  .append(
                    $('<button type="button" aria-label="Back" class="nav-link nav-page-back"/>')
                      .html("&lt;")
                      .click(function () {
                  ...
                  .append(
                    $('<button type="button" aria-label="Next" class="nav-link nav-page-next"/>')
                      .html("&gt;")
                      .click(function () {
                        core.navigatePage(element, 1);
                      })
                  ...
                  .append(
                    $('<button type="button" aria-label="now" class="nav-link nav-now"/>')
                      .html("&#9679;")
                      .click(function () {
                        core.navigateTo(element, "now");
                      })
                  ...
                  .append(
                    $('<button type="button" aria-label="Previous week" class="nav-link nav-prev-week"/>')
                      .html("&lt;&lt;")
                      .click(function () {
                  ...
                  .append(
                    $('<button type="button" aria-label="Previous Day" class="nav-link nav-prev-day"/>')
                      .html("&lt;")
                      .click(function () {
                        if (settings.scale === "hours") {
              ...
              .append(
                $('<div class="nav-slider-right" />')
                  .append(
                    $('<button type="button" aria-label="Nexy Day" class="nav-link nav-next-day"/>')
                      .html("&gt;")
                      .click(function () {
                        if (settings.scale === "hours") {
                          core.navigateTo(element, tools.getCellSize() * -4);
                  ...
                  .append(
                    $('<button type="button" aria-label="next week" class="nav-link nav-next-week"/>')
                      .html("&gt;&gt;")
                      .click(function () {
                        if (settings.scale === "hours") {
                          core.navigateTo(element, tools.getCellSize() * -8);
                  ...
                  .append(
                    $('<button type="button" aria-label="Zoom In" class="nav-link nav-zoomIn"/>')
                      .html("&#43;")
                      .click(function () {
                  ...
                  .append(
                    $('<button type="button" aria-label="Zoom Out" class="nav-link nav-zoomOut"/>')
                      .html("&#45;")
                      .click(function () {
                        core.zoomInOut(element, 1);
                      })
          ...
          ganttNavigate = $('<div class="navigate" />')
            ...
            .append(
              $('<button type="button" aria-label="Begin" class="nav-link nav-begin"/>')
                .html("&#124;&lt;")
                .click(function () {
            ...
            .append(
              $('<button type="button" aria-label="Next Day" class="nav-link nav-next-day"/>')
                .html("&gt;")
                .click(function () {
            ...
            .append(
              $('<button type="button" class="nav-link aria-label="Next Week" nav-next-week"/>')
                .html("&gt;&gt;")
                .click(function () {
                  core.navigateTo(element, tools.getCellSize() * -7);
                })
            ...
            .append(
              $('<button type="button" aria-label="End" class="nav-link nav-end"/>')
                .html("&gt;&#124;")
                .click(function () {
      ...
      wheelScroll: function (element, e) {
        ...
        var delta = "detail" in e ? e.detail : "wheelDelta" in e.originalEvent ? (-1 / 120) * e.originalEvent.wheelDelta : e.originalEvent.deltaY ? e.originalEvent.deltaY / Math.abs(e.originalEvent.deltaY) : e.originalEvent.detail;
