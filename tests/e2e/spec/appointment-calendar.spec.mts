...
test('opens booking form only for selectable appointment availability', async ({ page, request }) => {
  ...
  await expect(page.locator('.appointment-calendar-target-switcher .select2')).toBeVisible();
  await expect(page.locator('[data-appointment-event-type="availability"]')).toHaveCount(2);
