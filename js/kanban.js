...
(function () {
  window.GLPIKanban = function () {
    ...
    var initParams = function (args) {
      var overridableParams = ["element", "max_team_images", "team_image_size", "item", "supported_itemtypes", "allow_add_item", "allow_add_column", "dark_theme", "background_refresh_interval", "column_field", "allow_modify_view", "limit_addcard_columns", "allow_order_card", "allow_create_column"];
      if (args.length === 1) {
    ...
    var build = function () {
      ...
      var add_itemtype_bulk_link = '<a href="#">' + '<i class="fas fa-list" aria-hidden="true"></i>' + __("Bulk add") + "</a>";
      ...
      if (self.allow_modify_view) {
        overflow_dropdown += "<li class='kanban-remove' data-forbid-protected='true'>" + '<i class="fas fa-trash-alt" aria-hidden="true"></i>' + __("Delete") + "</li>";
        //}
      }
    ...
    var registerEventListeners = function () {
      ...
      if (!self.allow_order_card) {
        $(self.element).on("mouseenter", ".kanban-column", function () {
        ...
        $(self.element).on("mouseleave", ".kanban-column", function () {
          if (self.is_sorting_active) {
            return; // Do not change readonly states if user is sorting elements
          }
    ...
    var getTeamBadge = function (teammember) {
      ...
      if (self.team_badge_cache[itemtype] === undefined || self.team_badge_cache[itemtype][items_id] === undefined) {
        if (itemtype === "User") {
          var user_img = null;
          $.ajax({
        ...
        if (column["items"] !== undefined) {
          $.each(column["items"], function (card_id, card) {
            if (card["_team"] !== undefined) {
              Object.values(card["_team"])
                .slice(0, self.max_team_images)
    ...
    var generateOtherBadge = function (teammember, icon) {
      ...
      return (
        "<span class='fa-stack fa-lg' style='font-size: " +
        self.team_image_size / 2 +
        "px'>\
                     <i class='fas fa-circle fa-stack-2x' style='color: " +
        bg_color +
        "' title='" +
        teammember["name"] +
        "'></i>\
    ...
    var updateColumnState = function () {
      ...
      $(self.element + " .kanban-column").each(function (i, element) {
        ...
        if (self.user_state.state[i] === undefined || column_id !== self.user_state.state[i]["column"] || self.user_state.state[i]["folded"] !== column.hasClass("collapsed")) {
        ...
        $.each(column.find(".kanban-item"), function (i2, element2) {
          ...
          if (self.user_state.state[i] !== undefined && self.user_state.state[i]["cards"] !== undefined && self.user_state.state[i]["cards"][i2] !== undefined && self.user_state.state[i]["cards"][i2] !== new_state.state[i]["cards"][i2]) {
            new_state.is_dirty = true;
          }
        });
      });
