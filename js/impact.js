...
var GLPIImpact = {
  ...
  undo: function () {
    ...
    switch (action.code) {
      ...
      case this.ACTION_MOVE:
        this.cy.filter("node" + this.makeIDSelector(data.node)).position({
          x: data.oldPosition.x,
          y: data.oldPosition.y,
        });

        if (data.newParent !== null) {
          this.cy.filter("node" + this.makeIDSelector(data.node)).move({ parent: null });
        }
        break;
      ...
      case this.ACTION_ADD_COMPOUND:
        data.children.forEach(function (id) {
          GLPIImpact.cy.filter("node" + GLPIImpact.makeIDSelector(id)).move({ parent: null });
        });
      ...
      case this.ACTION_REMOVE_FROM_COMPOUND:
        if (data.children.length <= 2) {
          ...
          data.children.forEach(function (childId) {
            GLPIImpact.cy.getElementById(childId).move({ parent: data.compoundData.id });
          });
        } else {
          // Add the node that was removed
          this.cy.getElementById(data.nodeData.id).move({ parent: data.compoundData.id });
        }
      ...
      case this.ACTION_EDIT_IMPACT_VISIBILITY:
        ...
        $(GLPIImpact.selectors.toggleImpact).prop("checked", !$(GLPIImpact.selectors.toggleImpact).prop("checked"));
        break;

      // Toggle depends visibility
      case this.ACTION_EDIT_DEPENDS_VISIBILITY:
        this.toggleVisibility(this.BACKWARD);
        $(GLPIImpact.selectors.toggleDepends).prop("checked", !$(GLPIImpact.selectors.toggleDepends).prop("checked"));
        break;
      ...
      case this.ACTION_EDIT_DEPENDS_COLOR:
        ...
        $(GLPIImpact.selectors.dependsColor).val(GLPIImpact.edgeColors[GLPIImpact.BACKWARD]);
        this.updateStyle();
      ...
      case this.ACTION_EDIT_IMPACT_COLOR:
        ...
        $(GLPIImpact.selectors.impactColor).val(GLPIImpact.edgeColors[GLPIImpact.FORWARD]);
        this.updateStyle();
        this.cy.trigger("change");
        break;
      ...
      case this.ACTION_EDIT_IMPACT_AND_DEPENDS_COLOR:
        ...
        $(GLPIImpact.selectors.impactAndDependsColor).val(GLPIImpact.edgeColors[GLPIImpact.BOTH]);
        this.updateStyle();
        this.cy.trigger("change");
        break;
  ...
  redo: function () {
    ...
    switch (action.code) {
      ...
      case this.ACTION_MOVE:
        ...
        if (data.newParent !== null) {
          this.cy.filter("node" + this.makeIDSelector(data.node)).move({ parent: data.newParent });
        }
        break;
      ...
      case this.ACTION_ADD_COMPOUND:
        ...
        data.children.forEach(function (id) {
          GLPIImpact.cy.filter("node" + GLPIImpact.makeIDSelector(id)).move({ parent: data.data.id });
        });
        this.updateFlags();
      ...
      case this.ACTION_REMOVE_FROM_COMPOUND:
        if (data.children.length <= 2) {
          ...
          data.children.forEach(function (childId) {
            GLPIImpact.cy.getElementById(childId).move({ parent: null });
          });

          this.cy.getElementById(data.compoundData.id).remove();
        } else {
          // Remove only he node that was re-added
          this.cy.getElementById(data.nodeData.id).move({ parent: null });
        }
      ...
      case this.ACTION_EDIT_IMPACT_COLOR:
        ...
        $(GLPIImpact.selectors.forwardColor).val("set", GLPIImpact.edgeColors[GLPIImpact.FORWARD]);
        this.updateStyle();
        this.cy.trigger("change");
        break;
    ...
    while (this.no_positions.length !== 0) {
      this.no_positions.forEach(function (node, index) {
        ...
        node.neighborhood().forEach(function (ele) {
          ...
          if (not_placed.indexOf(ele.data("id")) === -1 || np_valid.indexOf(ele.data("id")) !== -1) {
            valid = true;
          }
        });
      ...
      while (!success) {
        ...
        while (angle !== -75) {
          ...
          GLPIImpact.cy.filter().forEach(function (ele) {
            ...
            if (bdb.x1 - 20 < position.x && bdb.x2 + 20 > position.x && bdb.y1 - 20 < position.y && bdb.y2 + 20 > position.y) {
              available = false;
            }
          });
  ...
  computeCompoundsDelta: function (currentCompounds) {
    ...
    Object.keys(GLPIImpact.initialState.compounds).forEach(function (compoundID) {
      ...
      if (Object.prototype.hasOwnProperty.call(currentCompounds, compoundID)) {
        ...
        // Check for updates ...
        if (compound.name != currentCompound.name || compound.color != currentCompound.color) {
  ...
  getContextMenuItems: function () {
    return [
      {
        id: "goTo",
        content: '<i class="fas fa-link" aria-hidden="true"></i>' + __("Go to"),
      ...
      {
        id: "showOngoing",
        content: '<i class="fas fa-list" aria-hidden="true"></i>' + __("Show ongoing tickets"),
      ...
      {
        id: "editCompound",
        content: '<i class="fas fa-edit" aria-hidden="true"></i>' + __("Group properties..."),
      ...
      },
      {
        ...
        content: '<i class="fas fa-external-link-alt" aria-hidden="true"></i>' + __("Remove from group"),
      ...
      {
        id: "delete",
        content: '<i class="fas fa-trash" aria-hidden="true"></i>' + __("Delete"),
  ...
  getEditCompoundDialog: function (compound) {
    ...
    var buttonSave = {
      ...
      click: function () {
        // Save compound name
        compound.data("label", $(GLPIImpact.selectors.compoundName).val());

        // Save compound color
        compound.data("color", $(GLPIImpact.selectors.compoundColor).val());
    ...
    return {
      ...
      close: function () {
        ...
        if (GLPIImpact.eventData.newCompound != null) {
          ...
          GLPIImpact.addToUndo(GLPIImpact.ACTION_ADD_COMPOUND, _.cloneDeep(GLPIImpact.eventData.newCompound));
   ...
   */
  prepareNetwork: function (impactContainer, colors, startNode) {
    // Set container
    ...
    if (params.impactcontexts_id !== undefined && params.impactcontexts_id !== 0) {
      ...
      if (params.zoom != "0") {
        ...
        GLPIImpact.cy.nodes().forEach(function (node) {
          if (node.position().x > viewport.x1 && node.position().x < viewport.x2 && node.position().y > viewport.x1 && node.position().y < viewport.x2) {
            empty = false;
          }
        });
  ...
  updateFlags: function () {
    ...
    this.cy.$("node:childless").forEach(function (node) {
      ...
      var distanceBA = otherNodeDijkstra.distanceTo(GLPIImpact.makeIDSelector(GLPIImpact.startNode));
    ...
    GLPIImpact.cy.$(GLPIImpact.makeIDSelector(GLPIImpact.startNode)).data("flag", this.BOTH);
    ...
    // always shown
    GLPIImpact.cy.$(GLPIImpact.makeIDSelector(GLPIImpact.startNode)).data("flag", this.DEFAULT);
  ...
  insertGraph: function (graph, params, startNode) {
    ...
    var centerToClickVector = [startNode.x - center.x, startNode.y - center.y];
    ...
    if (!(mainBoundingBox.x1 > newGraphBoundingBox.x2 || newGraphBoundingBox.x1 > mainBoundingBox.x2 || mainBoundingBox.y1 > newGraphBoundingBox.y2 || newGraphBoundingBox.y1 > mainBoundingBox.y2)) {
      ...
      while (i < 50000) {
        directions.forEach(function (vector) {
          ...
          edges.forEach(function (edge) {
            ...
            if (GLPIImpact.getDistance(point, edge.a) + GLPIImpact.getDistance(point, edge.b) == GLPIImpact.getDistance(edge.a, edge.b)) {
              // Found intersection
              intersect = {
                x: point.x,
                y: point.y,
      ...
      if (intersect !== undefined) {
        ...
        var vector = [intersect.x - closest.x, intersect.y - closest.y];
    ...
    // Set undo/redo data
    var data = {
      edges: eles.edges().map(function (edge) {
        return edge.data();
      }),
      compounds: [],
      nodes: [],
  ...
  getNodeAt: function (position, filter) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i].boundingBox().x1 < position.x && nodes[i].boundingBox().x2 > position.x && nodes[i].boundingBox().y1 < position.y && nodes[i].boundingBox().y2 > position.y) {
  ...
  buildOngoingDialogContent: function (ITILObjects) {
    return this.listElements(__("Requests"), ITILObjects.requests, "ticket") + this.listElements(__("Incidents"), ITILObjects.incidents, "ticket") + this.listElements(__("Changes"), ITILObjects.changes, "change") + this.listElements(__("Problems"), ITILObjects.problems, "problem");
  },
  ...
  listElements: function (title, elements, url) {
    ...
    if (elements.length > 0) {
      ...
      elements.forEach(function (element) {
        ...
        html += '<li><a target="_blank" href="' + link + '">' + element.name + "</a></li>";
      });
      html += "</ul>";
    }
   ...
   */
  addCompoundFromSelection: _.debounce(
    function () {
      ...
      if (GLPIImpact.eventData.boxSelected.length < 2) {
        ...
        // Show edit dialog
        $(GLPIImpact.selectors.editCompoundDialog).dialog(GLPIImpact.getEditCompoundDialog(newCompound));
  ...
  deleteFromGraph: function (ele) {
    ...
    if (ele.isEdge()) {
      ...
      deleted.edges = deleted.edges.concat(
        ele
          .connectedEdges(function (edge) {
            ...
            if (GLPIImpact.eventData.multipleDeletion != null) {
              GLPIImpact.eventData.multipleDeletion.edges.forEach(function (deletedEdge) {
                if (deletedEdge.id == edge.data("id")) {
                  exist = true;
                }
              });
  ...
  checkBadgeHitboxes: function (renderedPosition, trigger, blank) {
    ...
    GLPIImpact.badgesHitboxes.forEach(function (badgeHitboxDetails) {
      ...
      if (bb.x1 < renderedPosition.x && bb.x2 > renderedPosition.x && bb.y1 < renderedPosition.y && bb.y2 > renderedPosition.y) {
        hit = true;
    ...
    switch (GLPIImpact.editionMode) {
      case GLPIImpact.EDITION_DEFAULT:
        if (GLPIImpact.eventData.lastClicktimestamp != null) {
          ...
          if (event.timeStamp - GLPIImpact.eventData.lastClicktimestamp < 500 && event.target == GLPIImpact.eventData.lastClickTarget) {
  ...
  onDoubleClick: function (event) {
    if (event.target.isParent()) {
      ...
      $(GLPIImpact.selectors.editCompoundDialog).dialog(GLPIImpact.getEditCompoundDialog(event.target));
  ...
  onFreeOn: function (event) {
    ...
    if (GLPIImpact.eventData.grabNodePosition.x !== event.target.position().x || GLPIImpact.eventData.grabNodePosition.y !== event.target.position().y) {
  ...
  onRemove: function (event) {
    if (event.target.isNode() && !event.target.isParent()) {
      var itemtype = event.target.data("id").split(GLPIImpact.NODE_ID_SEPERATOR)[0];
      ...
      if (itemtype == GLPIImpact.selectedItemtype) {
        ...
        GLPIImpact.searchAssets(GLPIImpact.selectedItemtype, JSON.stringify(GLPIImpact.getUsedAssets()), $(GLPIImpact.selectors.sideFilterAssets).val(), 0);
      }
    }
  },
  ...
  onKeyDown: function (event) {
    ...
    switch (event.which) {
      ...
      case 46:
        ...
        // Set undo/redo data
        GLPIImpact.addToUndo(GLPIImpact.ACTION_DELETE, GLPIImpact.eventData.multipleDeletion);
  ...
  onKeyUp: function (event) {
    switch (event.which) {
      ...
      case 16:
        ...
        if (GLPIImpact.eventData.previousEditionMode !== undefined && (GLPIImpact.editionMode == GLPIImpact.EDITION_ADD_EDGE || GLPIImpact.editionMode == GLPIImpact.EDITION_ADD_COMPOUND)) {
      ...
      case 17:
        // Return to previous edition mode if needed
        if (GLPIImpact.editionMode == GLPIImpact.EDITION_ADD_COMPOUND && GLPIImpact.eventData.previousEditionMode !== undefined) {
          GLPIImpact.setEditionMode(GLPIImpact.eventData.previousEditionMode);
          GLPIImpact.eventData.previousEditionMode = undefined;
        }
  ...
  onMousemove: _.throttle(function (event) {
    ...
    if (GLPIImpact.checkBadgeHitboxes(event.renderedPosition, false, false) && !GLPIImpact.eventData.showPointerForBadge) {
    ...
    } else if (GLPIImpact.eventData.showPointerForBadge && !GLPIImpact.checkBadgeHitboxes(event.renderedPosition, false, false)) {
      // Exiiting a badge hitbox
      GLPIImpact.eventData.showPointerForBadge = false;

      // Reset to previous cursor
      $(GLPIImpact.impactContainer).css("cursor", GLPIImpact.eventData.previousCursor);
    }
      ...
      case GLPIImpact.EDITION_ADD_NODE:
        ...
        if (node) {
          ...
          if (GLPIImpact.eventData.boundingBox != null) {
            ...
            if (GLPIImpact.eventData.boundingBox.x1 > event.position.x || GLPIImpact.eventData.boundingBox.x2 < event.position.x || GLPIImpact.eventData.boundingBox.y1 > event.position.y || GLPIImpact.eventData.boundingBox.y2 < event.position.y) {
        ...
        if (node != null) {
          ...
          GLPIImpact.eventData.tmpEles = event.cy.add([
            ...
            },
            {
              group: "edges",
              data: {
                id: GLPIImpact.makeID(GLPIImpact.EDGE, GLPIImpact.eventData.addEdgeStart, "tmp_node"),
                source: GLPIImpact.eventData.addEdgeStart,
                target: "tmp_node",
              },
            },
    ...
    switch (GLPIImpact.editionMode) {
      ...
      case GLPIImpact.EDITION_DELETE:
        ...
        if (event.target.isNode()) {
          ...
          event.cy
            .filter(sourceFilter + ", " + targetFilter)
            .data("todelete", 1)
            .select();
        }
        break;
  ...
  menuOnShowOngoing: function (event) {
    $(GLPIImpact.selectors.ongoingDialog).html(GLPIImpact.buildOngoingDialogContent(event.target.data("ITILObjects")));
  ...
  menuOnRemoveFromCompound: function (event) {
    var parent = GLPIImpact.cy.getElementById(event.target.data("parent"));
    ...
    $.ajax({
      ...
      success: function (data) {
        $.each(data.items, function (index, value) {
          ...
          if (isHidden) {
            str += '<i class="fas fa-eye-slash impact-res-hidden" aria-hidden="true"></i>';
          }
  ...
  getUsedAssets: function () {
    ...
    GLPIImpact.cy
      .nodes()
      .not(GLPIImpact.getHiddenSelector())
      .forEach(function (node) {
        if (node.isParent()) {
          return;
        }

        var nodeId = node.data("id").split(GLPIImpact.NODE_ID_SEPERATOR);
  ...
  initToolbar: function () {
    ...
    $(GLPIImpact.selectors.sideFilterAssets).on("input", function () {
      ...
      searchAssetsDebounced(GLPIImpact.selectedItemtype, JSON.stringify(GLPIImpact.getUsedAssets()), $(GLPIImpact.selectors.sideFilterAssets).val(), 0);
    });

    // Load more results on "More..." click
    $(GLPIImpact.selectors.sideSearchMore).on("click", function () {
      GLPIImpact.searchAssets(GLPIImpact.selectedItemtype, JSON.stringify(GLPIImpact.getUsedAssets()), $(GLPIImpact.selectors.sideFilterAssets).val(), ++GLPIImpact.addAssetPage);
    });
    ...
    $(document).on("mousedown", GLPIImpact.selectors.sideSearchResults + " p", function (e) {
      ...
      if (e.which !== 1 || $(e.target).hasClass("impact-res-disabled") || $(e.target).parent().hasClass("impact-res-disabled")) {
        return;
      }
    ...
    $(document).on("mouseup", function (e) {
      ...
      if (event.which == 2) {
        GLPIImpact.checkBadgeHitboxes(GLPIImpact.projectIntoViewport(e.clientX, e.clientY, true), true, true);
      }
      ...
      if (e.target.nodeName == "CANVAS") {
        ...
        GLPIImpact.addNode(GLPIImpact.eventData.addNodeStart.id, GLPIImpact.eventData.addNodeStart.type, GLPIImpact.projectIntoViewport(e.clientX, e.clientY, false));
      }
  ...
  initCanvasOverlay: function () {
    ...
    GLPIImpact.cy.on("render cyCanvas.resize", function () {
      ...
      GLPIImpact.cy.filter("node:childless:visible").forEach(function (node) {
        ...
        var greyscale = (Math.round(rgb.r * 299) + Math.round(rgb.g * 587) + Math.round(rgb.b * 114)) / 1000;
        ctx.fillStyle = greyscale >= 138 ? "#4e4e4e" : "white";

        // Print number
        ctx.font = 6 * GLPIImpact.cy.zoom() + "px sans-serif";
        ctx.fillText(node.data("badge").count, pos.x - 1.95 * GLPIImpact.cy.zoom(), pos.y + 2.23 * GLPIImpact.cy.zoom());
      });
    });
  },
};
