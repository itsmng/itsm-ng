...
final class Version20241212100413 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $transformFkTables = [
            ...
            'glpi_users' => [
                ['name' => 'FK_usersLocationsD7DB', 'origin' => 'locations_id', 'ref' => 'glpi_locations'],
                ['name' => 'FK_usersProfilesD7DB', 'origin' => 'profiles_id', 'ref' => 'glpi_profiles'],
                ['name' => 'FK_usersEntitiesD7DB', 'origin' => 'entities_id', 'ref' => 'glpi_entities'],
                ['name' => 'FK_usersUserstitlesD7DB', 'origin' => 'usertitles_id', 'ref' => 'glpi_usertitles'],
                ['name' => 'FK_usersUsercategoriesD7DB', 'origin' => 'usercategories_id', 'ref' => 'glpi_usercategories'],
                ['name' => 'FK_usersGroupsD7DB', 'origin' => 'groups_id', 'ref' => 'glpi_groups'],
                ['name' => 'FK_usersDefaultRequestTypeD7DB', 'origin' => 'default_requesttypes_id', 'ref' => 'glpi_requesttypes'],
            ],
        ];

        if (str_contains($platform, 'postgres')) {
            // PostgreSQL branch: apply FK transformations using PG syntax and guard against duplicate constraints
            $renameCandidates = [
                'users_id_tech' => 'tech_users_id',
                'groups_id_tech' => 'tech_groups_id',
                'users_id_editor' => 'editor_users_id',
                'users_id_validate' => 'validate_users_id',
                'users_id_lastupdater' => 'lastupdater_users_id',
                'users_id_recipient' => 'recipient_users_id',
                'users_id_approval' => 'approval_users_id',
                'users_id_guests' => 'guests_users_id',
                'guest_users_id' => 'users_id_guest',
            ];

            foreach ($transformFkTables as $tableName => $rows) {
                foreach ($rows as $row) {
                    $foreignKey = $row['origin'];
                    $name = $row['name'];
                    $ref = $row['ref'];

                    // Build candidate column names: original + potential renamed variant
                    $candidates = [$foreignKey];
                    if (array_key_exists($foreignKey, $renameCandidates)) {
                        $candidates[] = $renameCandidates[$foreignKey];
                    }
                    // de-duplicate
                    ...
                    $this->addSql(
                        ...
                        "      AND column_name = ANY (ARRAY['" . $candidatesList . "']);\n" .
                        "\n" .
                        "    IF col IS NOT NULL THEN\n" .
                        "        EXECUTE 'ALTER TABLE " . $tableName . " ALTER COLUMN '||quote_ident(col)||' TYPE INT USING '||quote_ident(col)||'::integer';\n" .
                        "        EXECUTE 'ALTER TABLE " . $tableName . " ALTER COLUMN '||quote_ident(col)||' DROP NOT NULL';\n" .
                        "        EXECUTE 'UPDATE " . $tableName . " SET '||quote_ident(col)||' = NULL WHERE '||quote_ident(col)||' = 0';\n" .
                        "\n" .
                        "        IF NOT EXISTS (\n" .
                        "            SELECT 1 FROM pg_constraint\n" .
                        "            WHERE conrelid = '" . $tableName . "'::regclass\n" .
                        "              AND conname = lower('" . $name . "')\n" .
                        "        ) THEN\n" .
                        "            EXECUTE 'ALTER TABLE " . $tableName . " ADD CONSTRAINT " . $name . " FOREIGN KEY ('||quote_ident(col)||') REFERENCES " . $ref . " (id)';\n" .
                        "        END IF;\n" .
                        "    END IF;\n" .
                        "END $$"
                    );
            ...
            // prevent MySQL-specific statements below from running on PostgreSQL
            return;
        }
        ...
        $this->addSql('ALTER TABLE glpi_entities CHANGE entities_id entities_id INT NULL DEFAULT NULL, CHANGE authldaps_id authldaps_id INT DEFAULT NULL, CHANGE calendars_id calendars_id INT DEFAULT NULL, CHANGE tickettemplates_id tickettemplates_id INT DEFAULT NULL, CHANGE changetemplates_id changetemplates_id INT DEFAULT NULL, CHANGE problemtemplates_id problemtemplates_id INT DEFAULT NULL, CHANGE entities_id_software entities_id_software INT DEFAULT 0');
        $this->addSql('UPDATE glpi_entities SET entities_id = NULL WHERE entities_id = 0');
        $this->addSql('UPDATE glpi_entities e LEFT JOIN glpi_entities p ON e.entities_id = p.id SET e.entities_id = NULL WHERE e.entities_id IS NOT NULL AND p.id IS NULL');
        ...
        $this->addSql('ALTER TABLE glpi_networkportethernets ADD CONSTRAINT FK_networkportethernetsItemsdevicenetworkcardsD7DB FOREIGN KEY (items_devicenetworkcards_id) REFERENCES glpi_items_devicenetworkcards (id)');
        ...
        $this->addSql('ALTER TABLE glpi_networkportfiberchannels ADD CONSTRAINT FK_networkportfiberchannelsItemsdevicenetworkcardsD7DB FOREIGN KEY (items_devicenetworkcards_id) REFERENCES glpi_items_devicenetworkcards (id)');
        ...
        $this->addSql('ALTER TABLE glpi_networkportwifis ADD CONSTRAINT FK_networkportwifisItemsdevicenetworkcardsD7DB FOREIGN KEY (items_devicenetworkcards_id) REFERENCES glpi_items_devicenetworkcards (id)');
        ...
        $this->addSql('ALTER TABLE glpi_ticketrecurrents ADD CONSTRAINT FK_ticketRecurrentEntitiesD7DB FOREIGN KEY (entities_id) REFERENCES glpi_entities (id)');
        ...
        $this->addSql('ALTER TABLE glpi_tickettasks CHANGE taskcategories_id taskcategories_id INT DEFAULT NULL, CHANGE users_id_editor users_id_editor INT DEFAULT NULL, CHANGE tasktemplates_id tasktemplates_id INT DEFAULT NULL');
        ...
        $this->addSql('ALTER TABLE glpi_ticketvalidations ADD CONSTRAINT FK_ticketvalidationValidateUsersD7DB FOREIGN KEY (users_id_validate) REFERENCES glpi_users (id)');
